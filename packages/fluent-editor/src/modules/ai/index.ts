...
export class AI {
  ...
  private _inputPlaceholder: string = '请输入内容'
  private _showOperationMenu: boolean = false
  inputValue: string = '' // 存储输入框的值
  SEND: string = '发送' // 发送按钮文字
  BREAK: string = '停止回答' // 取消按钮文字
  INSERT_TEXT: string = '插入内容'
  REGENERATE: string = '重新生成'
  CLOSE: string = '关闭'
  THINK_TEXT: string = '正在为您分析并总结答案'
  RESULT_HEADER_TEXT: string = '根据您的诉求，已为您解答，具体如下：'
  ...
  constructor(public quill: FluentEditor, public options: any) {
    ...
    this.resultMenuList = [
      { text: this.INSERT_TEXT, icon: INSERT_ICON },
      { text: this.REGENERATE, icon: REBUILD_ICON },
      { text: this.CLOSE, icon: MENU_CLOSE_ICON },
    ]
  ...
  private showSelectionBubble() {
    if (!this.selectionBubbleEl) {
      ...
      this.selectionBubbleEl.innerHTML = `${AI_ICON}<span>AI 智能</span>`
  ...
  private createAIInputIcon() {
    if (!this.aiIconEl) {
      ...
      const icon = AI_ICON.replaceAll('paint_linear_2', 'paint_linear_3')
  ...
  private createResultElement() {
    ...
    this.resultPopupHeaderEl.innerText = this.RESULT_HEADER_TEXT
  ...
  private createOperationMenuElements() {
    if(!this.menuContainerEl){
      ...
      this.operationMenuList.forEach(({ text, icon, rightIcon, id }) => {
          ...
          menuItem.innerHTML = `${icon}<span>${text}</span>${rightIcon ? rightIcon : ''}`
  ...
  private createInputElements() {
    ...
    this.inputPlaceholder = '请输入问题或“/”获取提示词'
    // 添加发送按钮
  ...
  private createThinkElements() {
    if(!this.thinkContainerEl) {
      ...
      this.thinkContainerEl.innerHTML = `<span class="ql-ai-input-pre-icon ql-ai-think-icon">${THINK_ICON}</span><span class="ql-ai-think-text">${this.THINK_TEXT}</span>`
      this.thinkBtnEl = document.createElement('div')
      this.thinkBtnEl.className = 'ql-ai-think-btn'
      this.thinkBtnEl.innerHTML = `${STOP_ICON}<span>${this.BREAK}</span>`
  ...
  set inputPlaceholder(value: string) {
    ...
    if (this.inputEl) {
      this.inputEl.placeholder = this._inputPlaceholder
    }
  }
  ...
  set showOperationMenu(value: boolean) {
    ...
    if (this.menuContainerEl) {
      this.menuContainerEl.style.display = this._showOperationMenu? 'flex' : 'none'
    }
    if (value) {
      this.inputPlaceholder = '向我提问/选择操作'
    }
  }
}
